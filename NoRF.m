 ...
 *
 * [Description]: to check the output sound with NO RF stage
 *******************************************************************
%}
...
msg1_shift_IF = All_Modsignals .* cos(2*pi*125*k*index*Ts);
...
%mixer stage:
%transmit the signal to IF stage
msg2_shift_IF = All_Modsignals .* cos(2*pi*175*k*index*Ts);
...
%mixer stage:
%transmit the signal to IF stage
msg3_shift_IF = All_Modsignals .* cos(2*pi*225*k*index*Ts);
...
%mixer stage:
%transmit the signal to IF stage
msg4_shift_IF = All_Modsignals .* cos(2*pi*275*k*index*Ts);
